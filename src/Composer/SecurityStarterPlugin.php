...

{
...

    {
        ...
        $this->io->write(sprintf(
            '<comment>  driver=%s mode=%s tier=%s</comment>',
            $options['driver'],
            $options['mode'],
            $options['tier']
        ));
...

    {
        ...
        $driver = InstallOptionResolver::normalizeDriver(
            $this->io->select(
                'Which auditing library would you like to use?',
                array_values(InstallOptionResolver::driverChoices()),
                0
            )
        );
        ...
        $mode = InstallOptionResolver::normalizeMode(
            $this->io->select(
                'Which runtime mode do you want to secure?',
                array_values(InstallOptionResolver::modeChoices()),
                0
            )
        );
        ...
        $tier = InstallOptionResolver::normalizeTier(
            $this->io->select(
                'Which security tier do you want?',
                array_values(SecurityTier::installChoices()),
                0
            )
        );
