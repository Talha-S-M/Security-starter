    ...
    @if (config('security.captcha.enabled', true) && function_exists('captcha_src'))
        <div class="field">
            ...
            <div class="captcha-wrap">
                <img
                    id="pitb-captcha-img"
                    src="{{ captcha_src($captchaProfile) }}"
                    alt="captcha"
                    class="captcha-image"
                >
                <button type="button" class="btn btn-secondary captcha-refresh" onclick="pitbRefreshCaptcha()">Refresh</button>
            </div>
        ...
        <script>
            function pitbRefreshCaptcha() {
                const img = document.getElementById('pitb-captcha-img');
